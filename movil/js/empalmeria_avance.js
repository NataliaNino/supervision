...
function insert_avance(tx){ //alert('insertar');	
		...
		for (k = 0; k < i+1; k++){	//alert(k);
			var de_punto = $("#punto_ini"+k).val();
			var a_punto = $("#punto_fin"+k).val();
			var odf = $("#odf"+k).val();
			var now = new Date();
			...
			tx.executeSql('INSERT INTO avance_obra (id_unico,tramo,constructor,supervisor,de_punto,a_punto,odf,fecha_registro, latitud, longitud, exactitud) values ("'+id_unico+'","'+id_tramo+'","'+id_constructor+'","'+id_usuario+'","'+hilos+'","'+span+'","'+abs_inicial+'","'+abs_final+'","'+instalados+'","'+supervisados+'","'+detallados+'","'+fecha_captura+'","'+myLatitud+'","'+myLongitud+'","'+myPrecision+'","'+imagenfo+'")');
